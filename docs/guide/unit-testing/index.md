...
The OpenBMC Web UI unit test framework uses the Vitest runner and relies on the
following libraries:
...
There are two testing strategies for testing a Pinia store, which include
testing store parts separately or testing a running store instance. Each
strategy has its pros and cons. Given the size of the store and the number of
developers that could potentially contribute to the project, the suggested
strategy is to `test store parts separately`.
...
- [How to unit test a pinia Store](https://pinia.vuejs.org/cookbook/testing.html)
